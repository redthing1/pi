...
export function packReleasePackages(packages, tarballDirectory) {
	...
	for (const pkg of packages) {
		...
		tarballs.set(pkg.name, isAbsolute(packed) ? packed : join(tarballDirectory, packed));
	}
	return tarballs;
}

export function installCodingAgentConsumer(directory, tarballs) {
	...
	run("bun", ["install", "--ignore-scripts", "--production"], { cwd: directory });
}
