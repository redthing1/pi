...
import { type Model, normalizeContext, type TranscriptContext } from "@earendil-works/pi-ai";
...
describe("ExtensionRunner", () => {
	...
	function createCompactionCheckEvent(context: TranscriptContext): SessionCompactionCheckEvent {
		return {
	...
	function createBeforeCompactEvent(): SessionBeforeCompactEvent {
		return {
			...
			sourceContext: normalizeContext({ systemPrompt: "system", messages: [] }),
			preparation: {
	...
	describe("compaction first-claim events", () => {
		...
		it("passes a defensive provider-neutral context snapshot", async () => {
			...
			expect(Object.isFrozen(observed?.context.messages)).toBe(true);
