...
function runAutoCompaction(harness: Harness): Promise<void> {
	harness.settingsManager.applyOverrides({ compaction: { enabled: true, reserveTokens: 50, keepRecentTokens: 1 } });
	return harness.session.prompt("z".repeat(harness.getModel().contextWindow * 4));
}
...
describe("automatic compaction cancellation regressions", () => {
	...
	it("does not start post-run auto-compaction after abort", async () => {
		...
		let startsAtAbort: number | undefined;
		harness.session.subscribe((event) => {
			if (event.type === "message_end" && event.message.role === "assistant") {
				startsAtAbort = harness.eventsOfType("compaction_start").length;
		...
		expect(startsAtAbort).toBeDefined();
		expect(harness.eventsOfType("compaction_start")).toHaveLength(startsAtAbort!);
	});
