...
- Added authoritative in-memory skill and prompt-template replacement, plus backend-owned path resolution for extensions that represent remote or virtual workspaces.

### Fixed

- Fixed idle prompt-cache warming rebuilding expired caches when its timer or an extension decision is delayed.
- Prevented experimental local servers from connecting to Radius unless an explicit relay credential is supplied.
- Fixed experimental internal source processes failing to start when tests run under Bun.

### Removed

- Removed the built-in `/share` command and session-upload integration. Explicit local HTML and JSONL export remain available.
- Removed upstream bug reporting and automatic crash recording from this fork.
