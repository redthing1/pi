...
export function buildSessionContext(
	...
	const compaction = contextEntries.find((entry): entry is CompactionEntry => entry.type === "compaction");
	const messages = [
		...(compaction?.systemMessage ? [compaction.systemMessage] : []),
...
export class SessionManager {
	...
	private _setSessionFile(sessionFile: string, preloadedFileEntries?: FileEntry[]): void {
		...
		if (existsSync(this.sessionFile)) {
			const entries = preloadedFileEntries ?? loadEntriesFromFile(this.sessionFile, this.persist);
	...
	private _assertValidCompactionAnchor(firstKeptEntryId: string, placement: CompactionPlacement): void {
		...
		if (
			entry.type === "compaction" ||
			(entry.type === "message" && (entry.message.role === "toolResult" || entry.message.role === "system")) ||
	...
	private static openWithPersistence(
		...
		if (cwdOverride === undefined && existsSync(resolvedPath)) {
			try {
				...
				preloadedFileEntries = loadEntriesFromFile(resolvedPath, persist);
