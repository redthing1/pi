...
export class ExtensionRunner {
	...
	createContext(): ExtensionContext {
		...
		return {
			...
			getSystemPromptOptions: () => {
				runner.assertActive();
				return normalizeBuildSystemPromptOptions(runner.getSystemPromptOptionsFn());
			},
		};
	}
	...
	async emitSessionCompactionCheck(
		...
		for (const { ext, handlers } of snapshotEventHandlers(this.extensions, "session_compaction_check")) {
			for (const handler of handlers) {
				try {
	...
	async emitBashLaunch(event: BashLaunchEvent): Promise<BashLaunchEventResult | undefined> {
		...
		for (const { handlers } of snapshotEventHandlers(this.extensions, "bash_launch")) {
			for (const handler of handlers) {
