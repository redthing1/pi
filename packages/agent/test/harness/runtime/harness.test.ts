...
describe("runtime Harness lane management", () => {
	...
	it("uses one stable opaque provider session id per lane", async () => {
		...
		expect(sessionIds[0]).toMatch(/^[0-9a-f-]{36}$/u);
		expect(sessionIds[1]).toBe(sessionIds[0]);
		expect(sessionIds[2]).toMatch(/^[0-9a-f-]{36}$/u);
		expect(sessionIds[2]).not.toBe(sessionIds[0]);
		expect(sessionIds).not.toContain(expect.stringContaining("shared-session"));
	});
