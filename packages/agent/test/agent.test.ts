...
describe("Agent", () => {
	...
	it("adopts an accepted raw replacement before dispatch", async () => {
		...
		const replacementSystem = { role: "system" as const, content: "replacement", timestamp: 0 };
		...
		agent = new Agent({
			...
			beforeInference: (prepared) => {
				if (prepared.replacementCount === 0) {
					return {
						action: "replace",
						context: { messages: [replacementSystem, replacementMessage], tools: [] },
					};
				}
			...
			streamFn: (_model, context) => {
				...
				expect(agent.state.messages).toEqual([replacementSystem, replacementMessage]);
	...
	it("keeps accepted replacement state when abort races with acceptance", async () => {
		...
		expect(agent.state.systemPrompt).toBe("replacement");
		expect(agent.state.messages[0]).toBe(replacementSystem);
		expect(agent.state.messages[1]).toBe(replacementMessage);
		const lastMessage = agent.state.messages[agent.state.messages.length - 1];
